...
<div class="form-group">
	...
	<div class="col-md-9 col-sm-6 col-xs-12">
		<input id="mobile_number" class="form-control col-md-7 col-xs-12" value="{{ $nUser->home_owner_id == NULL?(count($errors) > 0? old('mobile_number'):($nUser->mobile_number)):(count($errors) > 0?old('mobile_number'):$eHomeOwners[$nUser->home_owner_id]->member_mobile_no) }}" type="number" required="required" name="mobile_number">
	</div>
</div>
<div class="form-group">
	<label for="middle-name" class="control-label col-md-3 col-sm-3 col-xs-12">Email <span class="required">*</span></label>
	<div class="col-md-9 col-sm-6 col-xs-12">
		<input id="email" required="required" class="form-control col-md-7 col-xs-12"value="{{ $nUser->home_owner_id == NULL?(count($errors) > 0? old('email'):($nUser->email)):(count($errors) > 0?old('email'):$eHomeOwners[$nUser->home_owner_id]->member_email_address) }}" type="text" name="email">
	</div>
</div>
<div class="form-group">
